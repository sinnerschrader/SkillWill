...
public class UserService {
	...
	@Autowired
	public UserService(UserRepository userRepository, LdapService ldapService, SkillService skillService,
		SkillRepository skillRepository, FitnessScoreProperties fitnessScoreProperties) {
	...
	public List<User> getUsers(SkillSearchResult skillSearch, String company, String location)
		...
		if (skillSearch.isInputEmpty()) {
			...
			candidates = userRepository.findBySkills(skillNames).stream()
				...
				.sorted(Comparator.comparingDouble(User::getFitnessScoreValue).reversed()).collect(Collectors.toList());
		}

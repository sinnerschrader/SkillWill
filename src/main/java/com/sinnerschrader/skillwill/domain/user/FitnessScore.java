...
/**
 * Calculate how well a user fits into a searched skill set. The result can be on a scale from 0 (does not fit at all)
 * to 1 (perfect match)
 *
 * @author torree
 */
public class FitnessScore {
	...
	public FitnessScore(User user, Collection<Skill> searchedSkills, FitnessScoreProperties props) {
		...
		this.props = props;
		value = (double) Math.round(calculateValue() * 10000) / 10000;
	}
	...
	private double getAverageSkillLevelSearched() {
		return getSearchedPersonalSkills().stream().mapToInt(UserSkill::getSkillLevel).average().orElse(0);
	}

	private double getAverageSkillLevelUnsearched() {
		return getUnsearchedPersonalSkills().stream().mapToInt(UserSkill::getSkillLevel).average().orElse(0);
	}

	private double getAverageWillLevelSearched() {
		return getSearchedPersonalSkills().stream().mapToInt(UserSkill::getWillLevel).average().orElse(0);
	}

	private double getAverageWillLevelUnsearched() {
		return getUnsearchedPersonalSkills().stream().mapToInt(UserSkill::getWillLevel).average().orElse(0);
	}

	private Set<UserSkill> getSearchedPersonalSkills() {
		return user.getSkills(true).stream().filter(s -> getSearchedSkillNames(searchedSkills).contains(s.getName()))
	...
	private Set<UserSkill> getUnsearchedPersonalSkills() {
		var skillset = new HashSet<>(user.getSkills(true));

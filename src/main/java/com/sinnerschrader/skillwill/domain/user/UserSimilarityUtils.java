...
/**
 * Get n Persons that are similar to the reference user similar = low jaccard distance between users' skillsets
 *
 * @author torree
 */
public class UserSimilarityUtils {
	...
	public static List<User> findSimilar(User user, Collection<User> candidates, Integer count) {
		...
		return candidates.stream().filter(candidate -> !candidate.getId().equals(user.getId()))
			.sorted(new JaccardDistanceComparator(user)).limit(count != null ? count : Long.MAX_VALUE)

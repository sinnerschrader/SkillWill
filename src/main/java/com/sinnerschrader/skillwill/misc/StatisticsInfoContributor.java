...
public class StatisticsInfoContributor implements InfoContributor {
	...
	private void contributeHiddenSkillCount(Info.Builder builder) {
		builder.withDetail("skills_hidden", skillRepository.findAll().stream().filter(Skill::isHidden).count());
	}
	...
	private void contributeUsedSkillCount(List<User> users, Info.Builder builder) {
		var usedSkillCount = (int) users.stream().flatMap(user -> user.getSkills(true).stream()).map(UserSkill::getName)
			.distinct().count();
	...
	private void contributeUserSkills(List<User> users, Info.Builder builder) {
		var stats = users.stream().mapToInt(user -> user.getSkills(true).size()).summaryStatistics();

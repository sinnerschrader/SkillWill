...
public class SkillController {
	 ...
	 * create new skill
	 */
	@ApiOperation(value = "add skill", nickname = "add skill",
		notes = "add a skill; Caution: parameter name is NOT the new skill's ID")
	...
	@ApiImplicitParams({
		...
		@ApiImplicitParam(name = "hidden", value = "hide skill in search/suggestions", paramType = "form",
			defaultValue = "false"),
	...
	public ResponseEntity<String> addSkill(@RequestParam String name, @RequestParam(required = false) String description,
	...
	public ResponseEntity<String> deleteSkill(@PathVariable String skill,
		@CookieValue("_oauth2_proxy") String oAuthToken, @RequestParam(required = false) String migrateTo) {
		if (!sessionService.checkTokenRole(oAuthToken, Role.ADMIN)) {
			return new StatusResponseEntity("invalid session token or user is not admin", HttpStatus.FORBIDDEN);
		}
	...
	/**
	 * suggest next skill to enter -> This is not the autocomplete for skill search (see getSkills(true) for that) ->
	...
	@ApiResponses({ @ApiResponse(code = 200, message = "Success"), @ApiResponse(code = 500, message = "Failure") })
	@ApiImplicitParams({
		@ApiImplicitParam(name = "search", value = "Names of skills already entered, separated by comma",
			paramType = "query", required = false),
		@ApiImplicitParam(name = "count", value = "Count of recommendations to get", paramType = "query",
			defaultValue = "10") })
	...
	public ResponseEntity<String> getNext(@RequestParam(required = false) String search,
		...
		try {
			List<String> searchItems = StringUtils.isEmpty(search) ? Collections.emptyList()
				: List.of(search.split("\\s*,\\s*"));
			var suggestionSkills = skillService.getSuggestionSkills(searchItems, count);
			var suggestionJsons = suggestionSkills.stream().map(Skill::toJSON).collect(Collectors.toList());
	...
	/**
	 * get/suggest skills based on search query -> can be used for autocompletion when user started typing
	 */
	...
	@ApiImplicitParams({ @ApiImplicitParam(name = "search", value = "Name to search", paramType = "query"),
		@ApiImplicitParam(name = "exclude_hidden", value = "Do not return hidden skills", paramType = "query",
			defaultValue = "true"),
	...
	public ResponseEntity<String> getSkills(@RequestParam(required = false) String search,
		...
		var skillsArr = new JSONArray(skillService.findSkill(search, exclude_hidden, count).stream().map(Skill::toJSON)
	...
	public ResponseEntity<String> updateSkill(@PathVariable String skill, @RequestParam(required = false) String name,
		@RequestParam(required = false) String description, @RequestParam(required = false) Boolean hidden,
		@RequestParam(required = false) String subskills, @CookieValue("_oauth2_proxy") String oAuthToken) {

...
public class UserController {
  ...
  @RequestMapping(path = "/users/{user}/skills", method = RequestMethod.DELETE, consumes = MediaType.APPLICATION_FORM_URLENCODED_VALUE)
  public ResponseEntity<String> removeSkill(@PathVariable String user, @RequestBody MultiValueMap<String, String> reqBody) {

    if (CollectionUtils.isEmpty(reqBody.get("skill")) || StringUtils.isEmpty(reqBody.get("skill").get(0))) {
      return new ResponseEntity<>(new StatusJSON("request param 'skill' requested").toString(), HttpStatus.BAD_REQUEST);
    }

    if (CollectionUtils.isEmpty(reqBody.get("session")) || StringUtils.isEmpty(reqBody.get("session").get(0))) {
      return new ResponseEntity<>(new StatusJSON("request param 'session' requested").toString(), HttpStatus.BAD_REQUEST);
    }

    final String skill = reqBody.get("skill").get(0);
    final String sessionKey = reqBody.get("session").get(0);
  ...
  public ResponseEntity<String> getSimilar(@PathVariable String user,
    ...
    JSONArray arr = new JSONArray(similar.stream().map(Person::toJSON).collect(Collectors.toList()));

...
public class UserControllerTest {
	...
	public void testGetUsersHideHidden() throws JSONException {
		...
		var skillJsonArray = new JSONObject(response.getBody()).getJSONArray("results").getJSONObject(0)
			.getJSONArray("skills");
	...
	public void testGetUsersIgnoreNonAlphanumerics() throws JSONException {
		...
		assertEquals("Java",
			new JSONObject(response.getBody()).getJSONArray("searched").getJSONObject(0).getString("found"));
		assertEquals(1, new JSONObject(response.getBody()).getJSONArray("results").length());
		assertEquals("aaaaaa",
			new JSONObject(response.getBody()).getJSONArray("results").getJSONObject(0).getString("id"));
	...
	public void testModifiyMentorExisitingSkill() throws JSONException {
		assertEquals("Java", new JSONObject(userController.getUser("aaaaaa").getBody()).getJSONArray("skills")
			.getJSONObject(0).getString("name"));

		assertFalse(new JSONObject(userController.getUser("aaaaaa").getBody()).getJSONArray("skills").getJSONObject(0)
			.getBoolean("mentor"));

		assertEquals(HttpStatus.OK, userController
			.updateSkills("aaaaaa", "Java", "3", "3", true, "YWFhLmFhYUBleGFtcGxlLmNvbQ==|foo|bar").getStatusCode());

		assertTrue(new JSONObject(userController.getUser("aaaaaa").getBody()).getJSONArray("skills").getJSONObject(0)
			.getBoolean("mentor"));
	}

	@Test
	public void testModifySkillsHidden() {
		assertEquals(HttpStatus.BAD_REQUEST, userController
			.updateSkills("aaaaaa", "hidden", "0", "3", false, "YWFhLmFhYUBleGFtcGxlLmNvbQ==|foo|bar").getStatusCode());
		assertNull(userRepo.findByIdIgnoreCase("aaaaaa").getSkill("hidden", true));
	}

	@Test
	public void testModifySkillsLevelOverMax() {
		var response = userController.updateSkills("aaaaaa", "Java", "0", "4", false,
			"YWFhLmFhYUBleGFtcGxlLmNvbQ==|foo|bar");
		assertEquals(HttpStatus.BAD_REQUEST, response.getStatusCode());
	}

	@Test
	public void testModifySkillsLevelsZero() {
		var response = userController.updateSkills("aaaaaa", "Java", "0", "0", false,
			"YWFhLmFhYUBleGFtcGxlLmNvbQ==|foo|bar");
		assertEquals(HttpStatus.BAD_REQUEST, response.getStatusCode());
	}
	...
	public void testModifySkillsSkillLevelOutOfRange() {
		var response = userController.updateSkills("aaaaaa", "Java", "5", "0", false,
			"YWFhLmFhYUBleGFtcGxlLmNvbQ==|foo|bar");
		assertEquals(HttpStatus.BAD_REQUEST, response.getStatusCode());
		assertEquals(2, userRepo.findByIdIgnoreCase("aaaaaa").getSkills(true).get(0).getSkillLevel());
		assertEquals(3, userRepo.findByIdIgnoreCase("aaaaaa").getSkills(true).get(0).getWillLevel());
	...
	public void testModifySkillsSkillUnknown() {
		var response = userController.updateSkills("aaaaaa", "UnknownSkill", "0", "0", false,
			"YWFhLmFhYUBleGFtcGxlLmNvbQ==|foo|bar");
		assertEquals(HttpStatus.BAD_REQUEST, response.getStatusCode());
		assertEquals(2, userRepo.findByIdIgnoreCase("aaaaaa").getSkills(true).get(0).getSkillLevel());
		assertEquals(3, userRepo.findByIdIgnoreCase("aaaaaa").getSkills(true).get(0).getWillLevel());
	...
	public void testModifySkillsValid() {
		var response = userController.updateSkills("aaaaaa", "Java", "3", "0", false,
			"YWFhLmFhYUBleGFtcGxlLmNvbQ==|foo|bar");
		assertEquals(HttpStatus.OK, response.getStatusCode());
	...
	public void testModifySkillsWillLevelOutOfRange() {
		var response = userController.updateSkills("aaaaaa", "Java", "0", "5", false,
			"YWFhLmFhYUBleGFtcGxlLmNvbQ==|foo|bar");
		assertEquals(HttpStatus.BAD_REQUEST, response.getStatusCode());
		assertEquals(2, userRepo.findByIdIgnoreCase("aaaaaa").getSkills(true).get(0).getSkillLevel());
		assertEquals(3, userRepo.findByIdIgnoreCase("aaaaaa").getSkills(true).get(0).getWillLevel());
	...
	public void testSetMentorNewSkill() throws JSONException {
		assertEquals(HttpStatus.OK,
			userController.removeSkill("aaaaaa", "Java", "YWFhLmFhYUBleGFtcGxlLmNvbQ==|foo|bar").getStatusCode());

		assertEquals(0, new JSONObject(userController.getUser("aaaaaa").getBody()).getJSONArray("skills").length());
